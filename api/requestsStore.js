...
export async function withRequestsLock(fn) {
  const run = async () => {
    ...
    for (const r of rows) {
      ...
      await db.run(`
        INSERT INTO requests (ID, DATE, full_name, car_number, email, type_requests, type_PZ, type_PB, type_PZ_ready, type_PB_ready, hasFiles_PZ, hasFiles_PB, isVerified_PZ, isVerified_PB, verified_files, verified_sections, file_comments, folderName)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
